...
void UMeleeAttackerComponent::BeginPlay()
{
    ...
    for (const FMeleeAttackData& data : MeleeAttackChain->Attacks)
    {
        ...
        data.Montage,
        TEXT("Melee Attack Chain %s doesn't have a montage assigned on all attacks!"),
        *MeleeAttackChain.GetName());
    }

    MainSkeletalMesh = GetOwner()->FindComponentByTag<USkeletalMeshComponent>(MainSkeletalMeshComponentTag);
    FDebugUtil::QuitCheckf(
    MainSkeletalMesh,
    TEXT("Actor %s has no skeletal mesh with tag %s!"),
    *GetOwner()->GetName(),
    *MainSkeletalMeshComponentTag.ToString());
...
void UMeleeAttackerComponent::EndAttack()
{
    ...
    else
    {
        ...
        ChainLeniencyTimer,
        this,
        &ThisClass::EndChainLeniency,
        MeleeAttackChain->ChainLeniencyTime);
...
const FHitResult&)
{
    ...
    FHittableHitData hitData{ data.Damage, data.StaggerAmount, GetOwner(), hittable->GetOwner()->GetActorLocation(), Faction, data.AttackType };
